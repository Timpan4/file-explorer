...
Pull requests to `main` run `.github/workflows/ci.yml`, which executes `bun run typecheck`, `bun run test`, `bun run rust:test`, `bun run rust:check`, and `bun run build` on Windows and Ubuntu.
